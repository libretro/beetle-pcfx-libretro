...
void MDFNMP_InstallReadPatches(void)
{
 ...
 for(unsigned int x = 0; x < 8; x++)
  ...
  {
   if(EmulatedPCFX.InstallReadPatch)
    EmulatedPCFX.InstallReadPatch(chit->addr);
  }
#endif
}
...
{
#if 0
 if(EmulatedPCFX.RemoveReadPatches)
  EmulatedPCFX.RemoveReadPatches();
#endif
}
 ...
 while(sscanf(string, "%u %c %63s %63s %63s", &bytelen, &endian, address, operation, value) == 5 && passed)
 {
  ...
  for(unsigned int x = 0; x < bytelen; x++)
  {
   ...
   if(endian == 'B')
    ...
    shiftie = x * 8;
   value_at_address |= EmulatedPCFX.MemRead(v_address + x) << shiftie;
  }
#endif
